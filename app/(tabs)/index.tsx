...
export default function EventsScreen() {
  ...
  const visibleEvents = useMemo(() => {
    ...
    function getNextAppointmentTimestamp(event: EventRecord): number | null {
      const calendarAppointmentTs = calendarMatches[event.id]?.upcomingEvent?.start.getTime();
      if (Number.isFinite(calendarAppointmentTs)) return calendarAppointmentTs ?? null;

      const manualAppointmentTs = manualUpcomingByEventId[event.id];
      if (Number.isFinite(manualAppointmentTs)) return manualAppointmentTs;

      const fallbackAppointmentTs = fallbackAppointmentsByEventId[event.id]?.upcoming?.ts;
      if (Number.isFinite(fallbackAppointmentTs)) return fallbackAppointmentTs ?? null;

      return buildEventAppointmentPoint(event)?.ts ?? null;
    }

    return events
      .filter((event) => !isCompleteProject(event))
      .sort((a, b) => {
        const aTs = getNextAppointmentTimestamp(a);
        const bTs = getNextAppointmentTimestamp(b);
        const aIsUpcoming = aTs != null && aTs >= todayStartTs;
        const bIsUpcoming = bTs != null && bTs >= todayStartTs;

        if (aIsUpcoming && bIsUpcoming) return (aTs ?? 0) - (bTs ?? 0);
  ...
  }, [calendarMatches, events, fallbackAppointmentsByEventId, manualUpcomingByEventId]);

...
import { pullEventByEntryId, upsertEventToSheet } from '@/lib/sheets-sync';
...
export default function PricingScreen() {
  ...
  const { events, createEvent, updateEvent, setSelectedEventId, upsertEventFromRemote } = useEvents();
      ...
      void (async () => {
        try {
          if (saveMode === 'new') {
            ...
            const saved = await upsertEventToSheet(SHEET_SYNC_CONFIG, eventToSave);
            let next = saved;
            ...
            upsertEventFromRemote({ ...next, id });
            setSelectedEventId(id);
            setSaveStatus(`Created "${formatEventOptionLabel(next)}" in sheet.`);
            router.push(`/event/${id}`);
          } else if (selectedExistingEvent) {
            const patch = buildPricingPatch(selectedExistingEvent);
            const eventToSave: EventRecord = {
              ...selectedExistingEvent,
              ...patch,
            };
            updateEvent(selectedExistingEvent.id, patch);
            const saved = await upsertEventToSheet(SHEET_SYNC_CONFIG, eventToSave);
            let next = saved;
            const savedEntryId = saved.entryId.trim();
            if (savedEntryId) {
            ...
            upsertEventFromRemote({ ...next, id: selectedExistingEvent.id });
            setSelectedEventId(selectedExistingEvent.id);
            setSaveStatus(`Saved "${formatEventOptionLabel(selectedExistingEvent)}" to sheet.`);
            router.push(`/event/${selectedExistingEvent.id}`);
          }
        } catch (error) {
          setSaveStatus(error instanceof Error ? error.message : 'Failed to save pricing changes to sheet.');
        } finally {

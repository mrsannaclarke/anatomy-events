...
export async function onRequest({ request, env, waitUntil }) {
  ...
  try {
    ...
    if (!currentEvent.ok) return response({ ok: false, error: currentEvent.error, retryable: currentEvent.retryable }, currentEvent.status);
    let uploadResult;
    if (currentEvent.result.event?.artImageUrl) {
      uploadResult = { ok: true, result: { entryId: job.entryId, artUrl: currentEvent.result.event.artImageUrl } };
    } else {
